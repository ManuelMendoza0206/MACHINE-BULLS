...
**Estado:** fuente de verdad de *estructura* del Sprint 1. Derivado de las specs de
...
**38 tareas + 3 epics = 41** para el *proyecto* en Sprint 1 (cifra de `sprint-1-task-list.md`).
De esas, **4 son de Jaicel en el repo backend** (`86e3122fr/fv/fy/g5` — modelos SQLAlchemy,
Pydantic, Alembic, trigger). En **este repo (frontend)**: 34 tareas + 3 epics.
...
| `86e301de2` | "Los **6 requisitos** de accesibilidad de **§2.5**" | "Los requisitos de a11y de la Requirement *Accessibility compliance* cubiertos por test (aria-disabled Button, aria-busy Skeleton, role Progress, texto-no-solo-color Badge)" |
...
| ID | Corrección |
|---|---|
| `86e301dea` | Ref `CLAUDE.md §3.3` → **§5** (jerarquía de errores). `src/lib/errors.ts` **ya existe** (scaffold) → la tarea es *tests* + verificar contra la spec, no crear de cero. |
